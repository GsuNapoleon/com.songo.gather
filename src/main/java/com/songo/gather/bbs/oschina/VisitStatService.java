...
/**
 * <p>decription:浏览记录数的实现代码</p>
 * <p>date:2014年6月17日 下午6:14:15</p>
 * 
 * @author gsu·napoleon
 */
public class VisitStatService extends TimerTask {
	...
	// 内存队列
	private final static Map<Byte, Map<Long, Integer>> 
		queues = new ConcurrentHashMap<Byte, Map<Long, Integer>>() {
	...
	public static void record(byte type, long obj_id) {
		Map<Long, Integer> queue = queues.get(type);
		if (queue != null) {
	...
	public void run() {
		for (byte type : TYPES) {
			Map<Long, Integer> queue = queues.remove(type);
	 ...
	 * @param queue
	 */
	private void _flush(byte type, Map<Long, Integer> queue) {
		if (queue.size() == 0)
			return;
		switch (type) {
		// 数据写入数据库....
